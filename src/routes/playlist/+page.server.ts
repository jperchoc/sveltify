...
export const actions: Actions = {
    addItem: async ({request, cookies, url, fetch}) => {
        ...
        if (!res.ok) {
            throw redirect(303, redirectTo ? `${redirectTo}?error=${res.statusText}` :  `/playlists/${playlist}?error=${res.statusText}`)
        } 

        if (redirectTo)  {
            throw redirect(303, `${redirectTo}?success=Track added successfully`);
        } else {
            throw redirect(303, `/playlists/${playlist}?success=Track added successfully`);
        }
    }    
};